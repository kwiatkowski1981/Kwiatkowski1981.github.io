...
console.log(` %c Hi there. It's nice to see You here.`, "color: yellow");
...
fetch(
    ...
    .then(resp => {
        ...
        for (const repo of repos) {
            const {description, name, html_url, homepage} = repo;
            const showDesription = description !== null ? description : "";

            const liElement = document.createElement('li');
            liElement.classList.add('api__list--element');
            const divApiMain = document.createElement('div');
            divApiMain.classList.add('api__main');
            const apiImageSvg = document.createElement('api__main-svg');
            apiImageSvg.innerHTML = `<svg  viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" class="api__logo">
            ...
            const apiHeading4 = document.createElement('h4');
            apiHeading4.classList.add('api__heading4');
            apiHeading4.textContent = `${name}`;
            const apiParagraph = document.createElement('p');
            apiParagraph.classList.add('api__paragraph');
            apiParagraph.textContent = `${showDesription}`;
            const apiFooterDiv = document.createElement('div');
            apiFooterDiv.classList.add('api__footer');
            const apiDemoLink = document.createElement('div');
            apiDemoLink.classList.add('api__demo__link');
            apiDemoLink.classList.add('api__nav--list1');
            apiDemoLink.innerHTML = `<a href="${homepage}" class="api__nav--list1" class="api__naw--list1-js" >
                         <p class="api__paragraph--link api__paragraph--link2">
                         <img src="../assets/img/demo1.svg" alt="small pc icon" class="api__demo"> Demo</p>
                         </a>`;

            const apiGithubLink = document.createElement('api__github__link');
            apiGithubLink.classList.add('api__github__link');
            apiGithubLink.classList.add('api__nav--list2');
            apiGithubLink.innerHTML = `<a href="${html_url}" class="api__nav--list2"> 
                           <p class="api__paragraph--link">
                           <img src="../assets/img/code1.svg" alt="code icon" class="api__code">  GitHub </p>
                           </a> `;

            list.appendChild(liElement);
            liElement.appendChild(divApiMain);
            divApiMain.appendChild(apiImageSvg);
